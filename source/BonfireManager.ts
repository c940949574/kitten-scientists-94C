...
export class BonfireManager implements Automation {
	...
	autoBuild(
		...
		builds: Partial<Record<BonfireItem, BonfireBuildingSetting>> = this.settings
			.buildings,
	) {
		...
		const builder = (build: ConcreteBuild) => {
			this.build(
				(build.name || build.id) as Building,
				build.stage ?? undefined,
				build.count,
			);
		};
	...
	autoUpgrade(context: FrameContext) {
		...
		if (
			...
			let energyConsumptionRate = this._host.game.workshop.get("cryocomputing")
				.researched
				? 1
				: 2;
			...
			if (
				...
				this._host.game.resPool.energyCons +
		...
		const amphitheatreMeta =
			this._host.game.bld.getBuildingExt("amphitheatre").meta;
	...
	autoGather(): void {
		for (
			let clicks = 0;
			clicks < Math.floor(this._host.engine.settings.interval / 20);
			++clicks
		) {
	...
	getBuild(name: Building, stage = 0) {
		...
		const meta = !isNil(buildingMeta.stages)
			? buildingMeta.stages[stage]
			: metaRaw;
		const controller =
			"stages" in meta
