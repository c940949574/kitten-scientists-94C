...
import {
	type SpaceBuildingSetting,
	SpaceSettings,
} from "./settings/SpaceSettings.js";
...
export class SpaceManager implements Automation {
	...
	autoBuild(
		...
		builds: Partial<Record<SpaceBuilding, SpaceBuildingSetting>> = this.settings
			.buildings,
	) {
	...
	autoUnlock(_context: FrameContext) {
		...
		const missions = this._host.game.space.meta[0].meta as Array<
			Required<UnsafeSpaceProgram>
		>;
		missionLoop: for (let i = 0; i < missions.length; i++) {
			...
			if (
				0 < missions[i].val ||
				!missions[i].unlocked ||
				!this.settings.unlockMissions.missions[missions[i].name as Mission]
					.enabled
			) {
				continue;
			}
			...
			for (const resource of prices) {
				...
				if (
					this._workshopManager.getValueAvailable(resource.name) < resource.val
				) {
