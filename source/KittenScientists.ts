...
export class KittenScientists {
	...
	constructor(
		...
		console.info(
			...cl(
				`Kitten Scientists ${ksVersion("v")} constructed. Checking for previous instances...`,
			),
		);
		if ("kittenScientists" in UserScriptLoader.window) {
			console.warn(
				...cl("Detected existing KS instance. Trying to unload it..."),
			);
	 ...
	 */
	validateGame() {
		BonfireSettings.validateGame(
			this.game,
			this.engine.bonfireManager.settings,
		);
		ReligionSettings.validateGame(
			this.game,
			this.engine.religionManager.settings,
		);
		ScienceSettings.validateGame(
			this.game,
			this.engine.scienceManager.settings,
		);
		SpaceSettings.validateGame(this.game, this.engine.spaceManager.settings);
		WorkshopSettings.validateGame(
			this.game,
			this.engine.workshopManager.settings,
		);
	}
	...
	run(): void {
		...
		this._gameBeforeSaveHandle = UserScriptLoader.window.dojo.subscribe(
			...
			(saveData: Record<string, unknown>) => {
				console.info(
					...cl("Injecting Kitten Scientists engine state into save data..."),
				);
		...
		this._serverLoadHandle = UserScriptLoader.window.dojo.subscribe(
			"server/load",
			(saveData: unknown) => {
				const state = UserScriptLoader.tryEngineStateFromSaveData(
					"ks",
					saveData,
				) as EngineState | undefined;
	...
	static decodeSettings(compressedSettings: string): EngineState {
		...
		if (compressedSettings.match(/\r?\n/)) {
			throw new InvalidOperationError(
				"Multi-line non-JSON input can't be decoded.",
			);
		}
	...
	setSettings(settings: EngineState) {
		...
		const requiresUiRebuild =
			this.engine.settings.ksColumn.enabled !==
			settings.engine.ksColumn.enabled;
	...
	async copySettings(settings = this.getSettings(), compress = true) {
		...
		await UserScriptLoader.window.navigator.clipboard.writeText(
			encodedSettings,
		);
	}
	...
	private _saveManager = {
		load: (saveData: Record<string, unknown>) => {
			console.info(
				...cl("Looking for Kitten Scientists engine state in save data..."),
			);
			...
			if (!state) {
				console.info(
					...cl("The Kittens Game save data did not contain a script state."),
				);
				return;
			}
