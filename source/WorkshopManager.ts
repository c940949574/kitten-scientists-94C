...
export class WorkshopManager extends UpgradeManager implements Automation {
	...
	async autoUnlock() {
		...
		workLoop: for (const [type, setting] of [
			...
			const upgrade =
				setting.$upgrade ??
				upgrades.find((subject) => subject.name === setting.upgrade);
			if (isNil(upgrade)) {
	 ...
	 */
	autoCraft(
		crafts: Partial<Record<ResourceCraftable, CraftSettingsItem>> = this
			.settings.resources,
	) {
		...
		for (const craft of Object.values(crafts)) {
			...
			const materials = Object.keys(
				this.getMaterials(craft.resource),
			) as Array<Resource>;
		...
		for (const [, request] of craftRequests) {
			for (const material of request.materials) {
				...
				material.consume =
					available / mustExist(billOfMaterials.get(material.resource)).length;
			}
		}
		...
		for (const [craft, request] of craftRequests) {
			...
			for (const material of request.materials) {
				...
				const orderDone = Math.max(
					0,
					Math.floor(Math.log(craftsDone) / Math.LN10 + 0.000000001),
				);
			...
			const availableTarget = this.getValueAvailable(craft.resource);
			const recipeProduces =
				1 + this._host.game.getResCraftRatio(craft.resource);
		...
		for (const [craft, request] of craftRequests) {
			...
			orders.push({
				amount: Math.floor(request.countRequested),
				name: craft.resource,
			});
		}
		if (0 < orders.length) {
	...
	craftMultiple(
		orders: Array<{ name: ResourceCraftable; amount: number }>,
	): void {
		...
		for (const order of orders) {
			...
			const resourceName = mustExist(
				this._host.game.resPool.get(order.name),
			).title;
	...
	getTickVal(resource: UnsafeResource): number | "ignore" {
		...
		if (resource.craftable) {
			...
			for (const [mat, amount] of objectEntries<Resource, number>(materials)) {
				const rat =
					(1 +
						this._host.game.getResCraftRatio(
							resource.name as ResourceCraftable,
						)) /
					amount;
		...
		if (
			production <= 0 &&
			(resource.name === "spice" || resource.name === "blueprint")
		) {
			return "ignore";
		}
	...
	getAverageHunt(): Partial<Record<Resource, number>> {
		...
		if (
			this._host.game.ironWill &&
			this._host.game.workshop.get("goldOre").researched
		) {
	...
	getPotentialCatnip(
		...
		if (worstWeather) {
			...
			productionField *=
				1 +
				this._host.game.getLimitedDR(
					this._host.game.getEffect("coldHarshness"),
					1,
				);
		} else {
			productionField *=
		...
		const productionVillager = resourceProduction.catnip
			? resourceProduction.catnip *
				(1 + this._host.game.getEffect("catnipJobRatio"))
			: 0;
		...
		const isWinterComing =
			this._host.game.challenges.currentChallenge === "winterIsComing";
		const paragonBonus = isWinterComing
			? 0
			: this._host.game.prestige.getParagonProductionRatio();
		...
		baseProd = mustExist(
			this._host.game.calendar.cycleEffectsFestival({ catnip: baseProd })
				.catnip,
		);
		...
		const unicornPastures =
			this._host.game.bld.getBuildingExt("unicornPasture").meta.val;
		baseDemand *=
			1 +
			this._host.game.getLimitedDR(
				pastures * -0.005 + unicornPastures * -0.0015,
				1.0,
			);

		// If we have any kittens and happiness over 100%.
		if (
			this._host.game.village.sim.kittens.length > 0 &&
			this._host.game.village.happiness > 1
		) {
	...
	refreshStock() {
		...
		for (const [name, resource] of objectEntries(
			this._host.engine.settings.resources.resources,
		)) {
