...
> {
	private _racePanels: Array<
		SettingsPanel<SettingLimitedTrigger, SettingLimitedTriggerListItem>
	>;
	...
	constructor(
			...
			}),
			{
				onRefreshRequest: () => {
					...
					this.settingItem.triggerButton.ineffective =
						settings.enabled &&
						settings.trigger < 0 &&
						Object.values(settings.races).some(
							(_) => _.enabled && _.trigger < 0,
						);
					...
					this.expando.ineffective =
						...
						Object.values(this._racePanels).some(
							(_) =>
								_.expando.ineffective ||
								_.settingItem.triggerButton.ineffective,
						);
				},
			},
		);
					...
					this.host.engine.i18n("option.crypto"),
					{
						...
						onRefreshRequest() {
							this.triggerButton.inactive =
								!this.setting.enabled || this.setting.trigger === -1;
						},
						onSetTrigger: async () => {
							const value = await Dialog.prompt(
								this,
		...
		listAddition.addChild(
			new EmbassySettingsUi(
				this,
				this.setting.buildEmbassies,
				locale,
				settings,
			),
		);
	...
	private _getTradeOption(
		...
		const element = new SettingLimitedTriggerListItem(
			parent,
			option,
			locale,
			label,
						...
						[
							option.trigger < 0
								? sectionSetting.trigger < 0
									? parent.host.engine.i18n("ui.trigger.section.blocked", [
											sectionLabel,
										])
		...
		const panel = new SettingsPanel(parent, option, element, {
			...
			new SeasonsList(parent, option.seasons, {
				onCheckSeason: (labelSeason: string) => {
					parent.host.engine.imessage("trade.season.enable", [
						ucfirst(label),
						labelSeason,
					]);
				},
				onUnCheckSeason: (labelSeason: string) => {
					parent.host.engine.imessage("trade.season.disable", [
						ucfirst(label),
						labelSeason,
					]);
				},
