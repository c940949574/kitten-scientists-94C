...
> {
	...
	constructor(
			...
			}),
			{
				onRefreshRequest: () => {
					...
					this.settingItem.triggerButton.ineffective =
						settings.enabled &&
						settings.trigger < 0 &&
						Object.values(settings.resources).some(
		...
		const preparedCrafts: Array<[CraftSettingsItem, string]> =
			this.host.game.workshop.crafts
				.filter(
					(item) =>
						!excludeCraftsArray.includes(item.name) &&
						!isNil(this.setting.resources[item.name]),
				)
				.map((resource) => [
					this.setting.resources[resource.name],
					ucfirst(resource.label),
				]);
		...
		for (const [option, label] of preparedCrafts) {
			...
			const element = new WorkshopCraftListItem(this, option, locale, label, {
				delimiter:
					option.resource === "kerosene" || option.resource === "blueprint",
				...
				onRefreshRequest: () => {
					...
					element.maxButton.ineffective =
						settings.enabled && option.enabled && option.max === 0;
