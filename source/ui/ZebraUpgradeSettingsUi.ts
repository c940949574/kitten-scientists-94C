...
import type { ZebraUpgradeSettings } from "../settings/UpgradeSettings.js";
...
export class ZebraUpgradeSettingsUi extends SettingsPanel<
	ZebraUpgradeSettings,
	SettingTriggerListItem
> {
	constructor(
		parent: UiComponent,
		settings: ZebraUpgradeSettings,
		locale: SettingOptions<SupportedLocale>,
	) {
		const label = parent.host.engine.i18n("ui.upgrade.upgradesZebra");
		super(
			parent,
			settings,
			...
			}),
			{
				onRefreshRequest: () => {
					...
					this.settingItem.triggerButton.ineffective =
						settings.enabled &&
						settings.trigger < 0 &&
						Object.values(settings.upgrades).some(
							(_) => _.enabled && _.trigger < 0,
						);

					this.expando.ineffective =
						settings.enabled &&
						!Object.values(settings.upgrades).some((_) => _.enabled);
				},
			},
		);

		const upgrades = this.host.game.workshop.zebraUpgrades.filter(
