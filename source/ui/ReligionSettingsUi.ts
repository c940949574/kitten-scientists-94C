...
> {
	...
	constructor(
			...
			}),
			{
				onRefresh: () => {
					for (const [
						buildingName,
						building,
					] of this._unicornBuildings.entries()) {
						...
						if (
							...
							building.elementLabel.removeClass(
								stylesTimeSkipHeatSettings.active,
							);
						}
					}
				},
				onRefreshRequest: () => {
					...
					for (const [_, building] of this._unicornBuildings.entries()) {
						...
						building.maxButton.readOnly =
							this._bestUnicornBuilding.setting.enabled;
						building.triggerButton.readOnly =
							this._bestUnicornBuilding.setting.enabled;
					}
				},
			},
		...
		const unicornsArray: Array<ZigguratUpgrade | "unicornPasture"> = [
			...UnicornItems,
		];
		...
		this._unicornBuildings = new Map([
			...
			...this.host.game.religion.zigguratUpgrades
				.filter(
					(item) =>
						unicornsArray.includes(item.name) &&
		...
		this.addChildrenContent([
			new SettingsList(this, {
				...
				new HeaderListItem(
					this,
					this.host.engine.i18n("$religion.panel.ziggurat.label"),
				),
				...
				new HeaderListItem(
					this,
					this.host.engine.i18n("$religion.panel.orderOfTheSun.label"),
				),
				...
				new HeaderListItem(
					this,
					this.host.engine.i18n("$religion.panel.cryptotheology.label"),
				),
						...
						label,
						{
							...
							onRefresh: () => {
								element.triggerButton.inactive =
									!this.setting[item].enabled ||
									this.setting[item].trigger === -1;
							},
							onSetTrigger: async () => {
