...
export type ReligionSettingsItems = Record<
...
export class ReligionSettings extends SettingTrigger {
	...
	static validateGame(game: GamePage, settings: ReligionSettings) {
		...
		for (const _ of missingInSettings) {
			console.warn(
				...cl(
					`The religion building '${_}' is not tracked in Kitten Scientists!`,
				),
			);
		}
		for (const _ of redundantInSettings) {
			...
			console.warn(
				...cl(
					`The religion building '${_}' is not a building in Kittens Game!`,
				),
			);
		}
	}
