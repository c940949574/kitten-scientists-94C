...
export class BonfireSettings extends SettingTrigger {
	...
	static validateGame(game: GamePage, settings: BonfireSettings) {
		...
		for (const _ of missingInSettings) {
			...
			console.warn(
				...cl(`The building '${_}' is not tracked in Kitten Scientists!`),
			);
		}
		for (const _ of redundantInSettings) {
			...
			console.warn(
				...cl(`The building '${_}' is not a building in Kittens Game!`),
			);
		}
	}
	...
	load(settings: Maybe<Partial<BonfireSettings>>) {
		...
		this.gatherCatnip.enabled =
			settings.gatherCatnip?.enabled ?? this.gatherCatnip.enabled;
		this.turnOnSteamworks.enabled =
			settings.turnOnSteamworks?.enabled ?? this.turnOnSteamworks.enabled;
		this.turnOnMagnetos.enabled =
			settings.turnOnMagnetos?.enabled ?? this.turnOnMagnetos.enabled;
		this.turnOnReactors.enabled =
			settings.turnOnReactors?.enabled ?? this.turnOnReactors.enabled;
