...
export class ReligionManager implements Automation {
		...
		if (this.settings.bestUnicornBuilding.enabled) {
			...
			const builds = Object.fromEntries(
				Object.entries(this.settings.buildings).filter(
					([, building]) =>
						building.variant !== UnicornItemVariant.UnicornPasture,
				),
			);
			// Build a unicorn pasture if possible.
			const maxPastures = negativeOneToInfinity(
	...
	getBestUnicornBuilding(): ZigguratUpgrade | "unicornPasture" | null {
		...
		const unicornsPerSecondBase =
			this._host.game.getEffect("unicornsPerTickBase") *
		...
		const religionRatio =
			this._host.game.getEffect("unicornsRatioReligion") + 1;
		// The ratio modifier through paragon.
		const paragonRatio =
			this._host.game.prestige.getParagonProductionRatio() + 1;
		...
		const unicornRiftChange =
			((this._host.game.getEffect("riftChance") * riftChanceRatio) /
				(10000 * 2)) *
			baseUnicornsPerRift;
		...
		const unicornsPerTickBase = mustExist(
			this._host.game.bld.getBuildingExt("unicornPasture").meta.effects
				?.unicornsPerTickBase,
		);
		...
		for (const building of validBuildings) {
			const buildingImpl = this._getBuild(
				building,
				UnicornItemVariant.Ziggurat,
			) as {
			...
			for (const effect in buildingInfo.effects) {
				if (effect === "unicornsRatioReligion") {
					religionBonus += mustExist(
						buildingInfo.effects.unicornsRatioReligion,
					);
				}
			...
			let riftBonus =
				((riftChance * riftChanceRatio) / (10000 * 2)) * unicornsPerRift;
			...
			if (amortization < bestAmortization) {
				if (
					0 < riftBonus ||
					(religionRatio < religionBonus && 0 < unicornPrice)
				) {
	...
	getBuildMetaData(builds: Partial<Record<FaithItem, ReligionSettingsItem>>) {
		...
		for (const build of Object.values(builds)) {
			...
			if (!this._getBuild(build.building, build.variant)) {
				...
				const model = mustExist(
					this._getBuild(build.building, build.variant),
				).model;
				const panel =
	...
	getUpgradeMeta(
		...
		switch (variant) {
			...
			case UnicornItemVariant.Cryptotheology:
				return (
					this._host.game.religion.getTU(name as TranscendenceUpgrade) ?? null
				);
		}
	...
	private async _autoSacrificeUnicorns() {
		...
		if (
			...
			negativeOneToInfinity(this.settings.sacrificeUnicorns.trigger) <=
				available &&
			negativeOneToInfinity(this.settings.sacrificeUnicorns.trigger) <=
				unicorns.value
		) {
	...
	private async _autoSacrificeAlicorns(context: FrameContext) {
		...
		if (
			...
			negativeOneToInfinity(this.settings.sacrificeAlicorns.trigger) <=
				available &&
			negativeOneToInfinity(this.settings.sacrificeAlicorns.trigger) <=
				alicorns.value
		) {
			...
			const controller =
				this._host.game.religionTab.sacrificeAlicornsBtn.controller;
	...
	private _autoTears(context: FrameContext) {
		...
		if (
			...
			const availableForConversion =
				available - this.settings.refineTears.trigger;
	...
	private async _autoTCs(context: FrameContext) {
		...
		if (
			...
			negativeOneToInfinity(this.settings.refineTimeCrystals.trigger) <=
				available &&
			negativeOneToInfinity(this.settings.refineTimeCrystals.trigger) <=
				timeCrystals.value
		) {
	...
	private _autoTAP() {
		...
		if (
			this.settings.transcend.enabled &&
			this.settings.autoPraise.trigger - 0.02 <= faithLevel
		) {
			this._autoTranscend();
		}
	...
	private _autoAdore(trigger: number) {
		...
		const transcendenceReached = mustExist(
			this._host.game.religion.getRU("transcendence"),
		).on;
		...
		const maxSolarRevolution =
			10 + this._host.game.getEffect("solarRevolutionLimit");
		...
		const worshipAfterAdore =
			0.01 +
			faith.value *
				(1 + this._host.game.getUnlimitedDR(epiphanyAfterAdore, 0.1) * 0.1);
	...
	private _autoTranscend() {
		...
		if (transcendenceReached) {
			...
			const needNextLevel =
				this._host.game.religion._getTranscendTotalPrice(
					transcendenceTierCurrent + 1,
				) -
				this._host.game.religion._getTranscendTotalPrice(
					transcendenceTierCurrent,
				);
			...
			if (epiphanyRecommend <= epiphany) {
				...
				const atheism = mustExist(
					this._host.game.challenges.getChallenge("atheism"),
				);
				atheism.calculateEffects?.(atheism, this._host.game);
				const blackObelisk = mustExist(
					this._host.game.religion.getTU("blackObelisk"),
				);
