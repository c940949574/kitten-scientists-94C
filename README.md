...
这行是按**你存档的实时状态**算的：当前座数、当前价格指数（含科技减免），取安全上限的 80% 向下取整。座数涨了、科技研了，数字会自己变。

### 网页版计算器

<https://c940949574.github.io/kitten-scientists-94C/>

和脚本内建议值同一套公式，额外支持自定义价格指数。单文件无依赖，下载仓库里的 `docs/index.html` 双击打开也能用。
...
- VER0014 在每个价格预算弹窗里实时显示「建议填写多少」，见上文
